...
const setup = (comparerMode) => {
  require([
    ...
    const polylines = JSON.parse(
      document.getElementById("polylines").textContent
    );

    if (comparerMode === "sideBySide") {
      const rightview = makeTileMap(
        data.newImageUrl,
        "right_mapview_container"
      );
      ...
      let polylineGraphics = addPolylinesToLayer(
        annotationLayerCopy,
        polylines
      );
    ...
    const submitAnnotationModal = async () => {
      return new Promise((resolve, reject) => {
        ...
        document
          .querySelector("#annotation-notes-submit")
          .addEventListener("click", () => {
            MicroModal.close("annotation-notes-modal");
            resolve();
          });
        document
          .querySelector("#annotation-notes-close")
          .addEventListener("click", () => {
            reject();
          });
      });
    ...
    annotationLayer.graphics.on("after-add", async (evt) => {
      ...
      const wkt_points = evt.item.geometry.rings[0]
        .map((pt) => `${pt[0]} ${pt[1]}`)
        .toString();
      ...
      await submitAnnotationModal().then(
        () => {
          const annotationData = new FormData(
            document.querySelector("#annotation-notes-form")
          );
          ...
          fetch(data.annotationPostUrl, {
            ...
            if (response.ok) {
              document.querySelector("#detections ul").textContent =
                "Refresh the page to view new change detection reports.";
              alert(
                "Your change detection was successfully stored in the database."
              );
            } else {
